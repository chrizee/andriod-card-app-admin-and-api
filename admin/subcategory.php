...
$subcategory = escape($Qstring);
$parent = $subCategoryObj->get(['id', '=', $subcategory], 'parent,status');
if($parent[0]->status == Config::get('status/deleted')) {
    Session::flash('home', "Select a valid sub category");
    Redirect::to("dashboard");
}
$subcategories = [];
if(!empty($parent[0]->parent)) {
    $subcategories = $subCategoryObj->get(['parent', '=', $parent[0]->parent, 'status', '=', Config::get('status/active')]);
}
?>
                ...
                <div class="col-md-3">
                    ...
                    <div class="box box-solid">
                        ...
                        <div class="box-body no-padding">
                            <ul class="nav nav-pills nav-stacked" id="subCategories">
                                ...
                                <?php if(!empty($subcategories)) {
                                    foreach ($subcategories as $key => $value) { ?>
                                        <li class="<?php if($subCategoryObj->getNameFromId(html_entity_decode($Qstring)) == $value->name) echo 'active ';
                                        ?>" title="<?php echo $value->id ?>">
                                            <a <?php echo (!empty($sublist)) ? "style=\"display: inline-block; width: 80%;\"": '' ?>" href="subcategory=<?php echo htmlentities($value->id);?>">
                                            <i class="fa fa-inbox"></i> <?php echo ucfirst($value->name)?>
                                            <span class="label label-primary"><?php echo $subCategoryObj->getTotal($value->id); ?></span>
                                            </a>
                                        </li>
                                    <?php }} else{ ?>
                                    <li><a href="#" ><i class="fa fa-exclamation"></i>No sub category</a></li>
                                <?php }?>
                            </ul>
                        </div>
                        <!-- /.box-body -->
                ...
                <?php if(!empty($Qstring)) { ?>
                    <div class="col-md-9 default">
                        <div class="box box-primary">
                            <div class="box-header with-border">
                                <h3 class="box-title"><?php echo $subCategoryObj->getNameFromId(escape($Qstring));?></h3>
                            ...
                            <div class="box-body">
                                <?php
                                if($subcategory) {
                                    $card = $cardObj->get(array('sub_category', '=', $subcategory, 'status', '=', Config::get('status/active')));
                                    $cat = $subCategoryObj->get(['id', '=', $subcategory, 'status', '=', Config::get('status/active')]);
                                    $folder = "sub category";
                                    ?>
                                    <div class="row">
                                        <div class="col-md-4">
                                        ...
                                        <div class="col-md-4 actionButtons" style="margin-top: 3em;">
                                            <a href="delete<?php echo str_replace(' ', '', $folder)."=".$cat[0]->id;?>">
                                                <button class="btn btn-sm btn-danger deleteCat" title="This will delete all cards in this subcategory">Delete <?php echo $folder; ?></button>
                                            </a>
                                            <p class="clearfix"></p>
                                    ...
                                    <?php } } else { ?>
                                    <p class="text-center">Select a valid sub category.</p>
                                <?php }?>
                            </div>
                            <!-- /. box -->
                        </div>
